...
class KDocFormatterTest {
  ...
  fun testPreTag3() {
    ...
    @Test
    fun testInlineSampleTag() {
        // From https://github.com/facebook/ktfmt/issues/406
        val source =
            """
           /**
        ...
        checkFormatter(
             ...
             * the activity or process recreation using the saved instance state
             * mechanism (for example it happens when the screen is rotated in the
             * Android application).
             *
             * @sample androidx.compose.runtime.saveable.samples.RememberSaveable
             *
             * If you use it with types which can be stored inside the Bundle then it
             * will be saved and restored automatically using [autoSaver], otherwise
             * you will need to provide a custom [Saver] implementation via the [saver]
             * param.
             *
             * @sample androidx.compose.runtime.saveable.samples.RememberSaveableCustomSaver
             *
             * You can use it with a value stored inside
             * [androidx.compose.runtime.mutableStateOf].
             *
             * @sample androidx.compose.runtime.saveable.samples.RememberSaveableWithMutableState
             *
             * If the value inside the MutableState can be stored inside the Bundle it
             * would be saved and restored automatically, otherwise you will need to
             * provide a custom [Saver] implementation via an overload with which has
             * `stateSaver` param.
             *
             * @sample androidx.compose.runtime.saveable.samples.RememberSaveableWithMutableStateAndCustomSaver
             ...
             *   restored.
             * @param key An optional key to be used as a key for the saved value. If
             *   not provided we use the automatically generated by the Compose runtime
             *   which is unique for the every exact code location in the composition
             *   tree
             * @param init A factory function to create the initial value of this state
             */
            """
